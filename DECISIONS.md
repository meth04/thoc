...
## 2026-07-12 — T13 spatial livelihood economy: quyết định thiết kế (ADR 0005)

| Ngày | Task | Quyết định | Lý do |
|---|---|---|---|
| 2026-07-12 | T13 | Cơ chế không gian/sinh kế mới (2 bờ, đò, khai hoang bờ kia, gà rừng commons, vụ đông, chăm trẻ, endowment food-equiv) là **optional overlay `spatial_v1.yaml`**, scenario-gated OFF qua `cfg.get("khong_gian.*", False)`, KHÔNG thêm key base `config/world.yaml`. | Giữ config-digest + `world_hash` + replay của MỌI run/scenario legacy BẤT BIẾN khi TẮT (đã test: 247 passed, 2-run-same-seed-same-hash, `bo` ngoài hash-struct). Bật chọn lọc cho nghiên cứu spatial. |
| 2026-07-12 | T13 | Calendar GIỮ 2 tick/năm; vụ đông (ngô/khoai) = lựa chọn canh mùa khô. BÁC phương án 3-mùa/tick. | Đổi đơn vị thời gian sẽ buộc chuyển đổi toàn bộ age/consumption/contract-duration/hazard/weather/reporting + phá hash mọi run. Vụ-đông-mùa-khô đạt "2 vụ/năm" mà không đụng thời gian (giản lược có chủ ý, ghi rõ là design_assumption). |
| 2026-07-12 | T13 | Xung đột CLAUDE.md điều luật #7 (cấm định chế có tên): đò/thuê đất/thuê thợ/chăm-trẻ-có-công **KHÔNG phải mã định chế riêng** — chúng là TỔ HỢP asset (thuyền) + hợp đồng (clause `gop_cong`/`qua_song`/`quyen_su_dung`) + ledger. Nhãn "lái đò"/"chủ đất" CHỈ do observatory dán (chỉ đọc). | Nhất quán với ADR 0001 §B (cổng định chế minh bạch) + charter Lớp-5: engine chỉ có vật lý + nguyên tố hợp đồng; nghề/định chế là outcome quan sát, không hard-code, không gán vĩnh viễn cho agent (anti-teleology). |

## 2026-07-13 — Tăng cường tái lập và sinh kế đa dạng

| Ngày | Quyết định | Lý do |
|---|---|---|
| 2026-07-13 | `world_hash` chuyển sang `behavioral-state-v2`: băm đầy đủ state làm tick/prompt tiếp theo rẽ nhánh (agent memory/di chúc/cư trú/prior giá, contract clauses, board, lịch sử giá, commons, policy cards, entity/blueprint, cửa sổ observatory). Chỉ loại event/metric journal và cache lười xác định hoàn toàn từ `(seed, config, tick)`. | Hash cũ bỏ sót state quan trọng nên hai thế giới hành vi khác nhau có thể cùng hash. Cache thời tiết/dịch không băm để một policy đọc-thuần không đổi hash. |
| 2026-07-13 | Transcript ghi cả outcome lỗi provider; replay tiêu thụ lỗi đó và `--verify` fail khi còn miss/entry thừa. | "Miss để tái tạo fallback" không phải lossless artifact, che lỗi prompt drift và làm overclaim replay real. |
| 2026-07-13 | Giá trước phiên chợ là prior chủ quan seeded theo agent trong `hanh_vi.gia_ky_vong`, sau đó cập nhật từ khớp lệnh; cấm fallback literal trong mind. | Giữ chợ là nơi duy nhất định giá, nhưng vẫn cho tác nhân đặt bid/ask khi chưa có giao dịch; mọi prior có provenance scenario và nằm trong hash. |
